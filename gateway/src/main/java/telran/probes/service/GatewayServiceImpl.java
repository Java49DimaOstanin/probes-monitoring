...
@Service
@Slf4j
public class GatewayServiceImpl implements GatewaySevice {
	...
	ResponseEntity<byte[]> res =	switch(httpMethod) {
		...
		case "DELETE" -> proxyExchange.uri(url).delete();
			default -> throw new IllegalArgumentException("Wrong H Method");
		};
		return res;
	}

	private String getUrl(HttpServletRequest request) {
		String uri = request.getRequestURI();// /sensors/emails/
